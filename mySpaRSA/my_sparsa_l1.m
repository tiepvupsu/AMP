function [x] = my_sparsa_l1(y, A, lambda, Rho, opts)
...
% Solving the ICR cost function using sparse method
    ...
    function c = calc_phi(x)
        c = 0.5*normF2(y - A*x) + 0.5*lambda*x'*x + lambda2*norm1(x);
    end 
    tolP = 1e-6;
    B = (A'*A + lambda*eye(size(A, 2)));
    Aty = A'*y;
...
%             x_new = sparsa_approx(u_t, 2*Rho/a_t, opts);
            opts1.lambda = lambda2/a_t;
            opts1.pos = opts.pos;
            x_new = shrinkage(u_t, opts1);
            %% M = 0 
